...
## question 3 (updated)
q3_cox1 <- coxph(Surv(tdfs, deltadfs) ~ age + male + cmv + donorage + donormale 
              + donorcmv + waittime + as.factor(hospital) + mtx + as.factor(disgroup) + fab, data = bmt) %>% tidy(conf.int = TRUE, conf.level = 0.9) 
...
# now remove mtx since we will do this separately 
q3_cox2 <- coxph(Surv(tdfs, deltadfs) ~ age + male + cmv + donorage + donormale 
                 + donorcmv + waittime + as.factor(hospital) + as.factor(disgroup) + fab, data = bmt) %>% tidy(conf.int = TRUE, conf.level = 0.9) 
...
# log rank for mtx
q3_dat_surv <- Surv(time = bmt$tdfs, event = bmt$deltadfs, type = "right")
survdiff(q3_dat_surv ~ mtx, data=bmt)
...
cox1 <- coxph(Surv(tdfs, deltadfs) ~ age + male + cmv + donorage + donormale 
              + donorcmv + waittime + as.factor(hospital) + mtx + as.factor(disgroup) + fab, data = subset_bmt) %>% tidy(conf.int = TRUE, conf.level = 0.9) 
...
# now remove mtx since we will do this separately 
cox2 <- coxph(Surv(tdfs, deltadfs) ~ age + male + cmv + donorage + donormale 
              + donorcmv + waittime + as.factor(hospital) + as.factor(disgroup) + fab, data = subset_bmt) %>% tidy(conf.int = TRUE, conf.level = 0.9) 
...
# log rank for mtx
dat_surv <- Surv(time = subset_bmt$tdfs, event = subset_bmt$deltadfs, type = "right")
survdiff(dat_surv ~ mtx, data=subset_bmt)
